<h3>IHRE RESERVATIONEN</h3>
    ...
    <table class='table'>
        @foreach($v as $p)
    <tr>
            <td width='300px' style='line-height:1'>{{$p->o_title}} <br> {{$p->o_subtitle}}</td>
          
            @if($currency=='CHF')
                <td style='padding-top:1    0px'><span style='color:#013a89;font-weight:bold;'>CHF {{$p->adult_price}} </span><span style="color:gray">EUR {{floor($p->adult_price/$exchange)}}*</span></td>
            @else
