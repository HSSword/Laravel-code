...
var br="http://www.itrendin.win/",offerType='alle',offerRegion='alle',offerDuration='alle',keyw='alle',vaucherCustom=100,activateEnter=false,jssor1_slider=null; 
...
function showImageVoucherSingle(image,vType){
    ...
    // $('#singleImageDiv').fadeIn();
    // $('#singleImageDiv').fadeOut();
    console.log('vType'+vType);
    console.log('image',image);
    $('#voucherType').val(vType);
}
...
function changeDetectorInput(){
  ...
  $( document ).on('keyup',"#exampleTextarea", function() {
      console.log("keyUp funciton", this.value);
      var txtVal = this.value;
...
$(document).ready(function () {
    ...
    var liberInd="";
    $( document ).on('click',"#voucherStepb1,#voucherStepb2,#voucherStepb3", function(eve) {
        var vStepId=$(eve.target).attr('id');
        console.log('vStepIdsss',vStepId);
        if(vStepId=='voucherStepb1')
        {
            var vFor=$('#vouchName').val();
            var vText=$('#exampleTextarea').val();
            var vPrice=$('#otherValue').val();
            console.log('vPrice',vPrice,'vText',vText,'vFor',vFor);
            var optionsValues=$('input[type=radio][name=optionsRadios]:checked');
        ...
        else if(vStepId=='voucherStepb2')
        {
            ...
            console.log('uShipping',uShipping);
            validateVoucherOffer(2,2,'#VouchForm2',{});
        ...
        {
            console.log('client email',$('#userEmailHolder').text());
      ...
      $( document ).on('click',"#bookingStepb1,#bookingStepb2,#bookingStepb3", function(eve) {
        ...
        {
            console.log('next',window.history);
        ...
        else if(vStepId=='bookingStepb3')
        {
            ...
            console.log('guaranteeCheck',guaranteeCheck);
            ...
            console.log('bookingsssss data to submit',sendData);
            ...
            $('#userShippingHolder').text(uShipping);
            $('#userStatusHolder').text(uStatus);
            console.log('herlakjdflsakf ',uStatus);
            if(uStatus=="Herr")
            {
            ...
            let pre_url=currFullUrl.replace(br+"/reservation", "");
            let push_url=pre_url.replace("step=3", "step=4");
      ...
      $( document ).on('click',"#bookingStepRet1,#bookingStepRet2,#bookingStepRet3", function(eve) {
        ...
        {
            //$('#bookingStep1').addClass('stepShowBookingSpecial');
            //$('#bookingStep1S2').addClass('stepShowBookingSpecial');
        ...
        else if(vStepId=='bookingStepRet3')
        {
            // $('.bookingSteps').removeClass('stepShowBooking');
            // $('.stepsIndic').removeClass('step1');
            // $('#step2I').addClass('step1');
            // $('#bookingStep2').addClass('stepShowBooking');
            // var currFullUrl = window.location.href; 
            // let pre_url=currFullUrl.replace(br+"/reservation", "");
            // let push_url=pre_url.replace("step=3", "step=2");
            // window.history.pushState(null, null, window.location);
            // console.log('push_url',push_url);
            // window.history.replaceState(null, null,push_url);
            window.history.back();
      ...
      $( document ).on('click',"#voucherOffers2,#voucherOffers3,#voucherOffers4", function(eve) {
        ...
        {
            console.log('next',pricesSelected);
            console.log('validate offerVouchForm1');
              //console.log('validate happening',$("#offerVouchForm1").validate());
              //$("#offerVouchForm1").validate();
            //   $("#offerVouchForm1").submit(    function(e){     
            //     console.log('submit errors',e);
            //     e.preventDefault();
            //     var $form = $(this);
        ...
        else if(vStepId=='voucherOffers3')
        {
            ...
            var uStatus=$('input[name=andreOp]:checked').val();
            console.log('getting option ',uStatus);
            var dbPay=0;
            var dbPayVal=0;
            var vFor=$('#vouchName').val();
            var vText=$('#exampleTextarea').val();
            ...
            console.log('booking data to submit',sendData);
            $('#userName').text(uName);
            $('#userSurname').text(uSurname);
            $('#uName').text(uSurname);
            $('#userFullnameHolder').text(uName+" "+uSurname);
        ...
        else
        {
            ...
            let confDataOffV=$('#vaucherConfirmationInfo').html();
    ...
    $(window).resize(function(){
        ...
        {
            console.log('popstate here',e);
            end_l_of_c_s=false;
            if (e.state !== null) 
            {
                /*this should check if the url is what we excpect*/
                var g_url=location.href;
                var o_a_wh="#pageDynamic";
                console.log('popstate link',g_url);
                ...
                console.log("wSearch",wSearch);
                let pathNew=window.location.pathname.replace('/','');
                ...
                else
                {
                    ...
                    var get_res=navigateMaster(g_url,{urlCorrect:1});
                }
            }
      });
...
function validateVoucherOffer(stIn,stNow,fId,fData){
    ...
    if(stIn==1)
    {
        ...
        console.log('cRuels',cRules);
        console.log('cMessages',cMessages);
        console.log('fId',fId);
        ...
        $(fId).validate({
            ...
            submitHandler: function(form) {
                console.log('submit handeled inside',stNow);
                if(stNow==1)
                {
                    $('.bookingSteps').removeClass('stepShowBooking');
                    $('.stepsIndic').removeClass('step1');
                    $('#step3I').addClass('step1');
                    console.log('post remove class',$('.bookingSteps'));
                    let token = $("input[name=_token]").val();
                    selectPrices(token);
                    $('#bookingStep3').addClass('stepShowBooking');
                    var currFullUrl = window.location.href; 
                    let pre_url=currFullUrl.replace(br+"/reservation", "");
                    let push_url=pre_url.replace("step=2", "step=3");
                    window.history.pushState(null, null, window.location);
                    console.log('push_url',push_url);
                    window.history.replaceState(null, null,push_url);
                }
    ...
    else if(stIn==2)
    {
        ...
        $(fId).validate({
            ...
            submitHandler: function(form) {
                ...
                if(stNow==1)
                {
                    let mvfs1=$('#mainVaucherFormS1').html();
                    console.log('form being sent',mvfs1);
                    let pData={
                        step:2,
                        imgForm:mvfs1
                    };
                    ...
                    $('#voucherStep2').addClass('stepShowVoucher');
                    var currFullUrl = window.location.href; 
                    let pre_url=currFullUrl.replace(br+"/geschenkgutschein", "");
                    var push_url='';
        ...
        $(fId).validate({
            ...
            submitHandler: function(form) {
                ...
                if(stNow==1)
                {
                    ...
                    var currFullUrl = window.location.href; 
                    //let pre_url=currFullUrl.replace(br+"/reservation", "");
                    let pre_url=currFullUrl.substr(currFullUrl.indexOf('?')+1);
                    if(currFullUrl==window.location.href)
                    {
                        pre_url='';
                    }
                    console.log('pre urljs',pre_url);
                    var push_url='';
                    if(pre_url.indexOf('step=1') !== -1)
                    {
                        push_url=pre_url.replace("step=1", "step=2");
                ...
                else if(stNow==2)
                {
                    ...
                    let mvfs1=$('#offerVaucherContact').html();
                    let pData={
                        step:3,
                        imgForm:mvfs1
                    };
...
function navigateMaster(cUrl,opt)
{
    ...
    console.log('fullUrlsssss ',fullUrl);
...
{
    $.ajax({
        url: br+'list-offers/region/'+offerRegion+'/offer_type/'+offerType+'/duration/'+offerDuration+'/keyword/'+keyw+'?navigate=1',
        context: document.body
      }).done(function(r) {
        //document.body=r;s
        if(r.view)
