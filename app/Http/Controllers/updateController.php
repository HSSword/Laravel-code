...
class updateController extends Controller
{
    public function uploadOfferPics(Request $r,$offerId){
        ...
        foreach($r->picture as $pic){
            ...
            $img_name=time().'.'.$ext;
            $path=public_path().'/assets/temp_images/'.$img_name;
            ...
            if($image->save($path)){
                ...
                if(substr($mime, 0, 5) == 'image'){
                    //upload image
                    if(in_array($ext,$acceptedFormats)){
                        $dirImage=public_path()."/assets/img/tx_travel";  
                    }else{
                        continue;
                    }
    ...
    public function uploadRegionPic($regionId){
        ...
        foreach($pics as $p){
            ...
            File::move(public_path().'/assets/temp_images/'.$p->name,public_path().'/assets/images/regionen/'.$p->name);
        }
        return true;
    }
    ...
    public function uploadCategoryPic($catId){
        ...
        foreach($pics as $p){
            ...
            $insertToSysFileReference=DB::table('sys_file_reference')->insert($data2);
            File::move(public_path().'/assets/temp_images/'.$p->name,public_path().'/assets/images/category/'.$catName."/".$p->name);
        }
        return true;
    }
    ...
    protected function insertPics($offer){
        ...
        $i=0;
        foreach($pics as $p){
            $path = public_path().'/assets/temp_images/'.$p->name;
            $path2 = public_path().'/assets/img/tx_travel/'.$p->name;
    ...
    public function submitEditCategory(Request $request){
        ...
        if($update || $changePic){
            ...
            Session::flash('fail','There were no changes to save.');
    ...
    public function submitEditOffer(Request $request){ 
        ...
        return redirect()->action('mainController@editOfferForm',['offer_id'=>$request->uid]); 
    }
    ...
    public function submitHomeMeta(Request $request){
        $titleMeta = $request->title_meta ? $request->title_meta : '';
        $descriptionMeta = $request->description_meta ? $request->description_meta : ''; 
        $keywordMeta = $request->keyword_meta ? $request->keyword_meta : ''; 
        $seo = $request->seo ? $request->seo : ''; 
        
        if($request->id > 0){
            $update1=DB::table('meta_tags')
                ->where('page','')
                ->update(['title_meta'=>$titleMeta, 'description_meta'=>$descriptionMeta, 'keyword_meta'=>$keywordMeta,'seo'=>$seo]);
        }else{
            $update1=DB::table('meta_tags')
                ->where('page','')
                ->insert(['title_meta'=>$titleMeta, 'description_meta'=>$descriptionMeta, 'keyword_meta'=>$keywordMeta,'seo'=>$seo]);
        }
       
        Session::flash('success','Home Meta Updated Successfully');
        return redirect()->back();
    }

    public function submitDefaultMeta(Request $request){
        $titleMeta = $request->title_meta ? $request->title_meta : '';
        $descriptionMeta = $request->description_meta ? $request->description_meta : ''; 
         $keywordMeta = $request->keyword_meta ? $request->keyword_meta : ''; 
        ...
        if($request->id > 0){
            $update1=DB::table('meta_tags')
                ->where('page','default')
                ->update(['title_meta'=>$titleMeta, 'description_meta'=>$descriptionMeta, 'keyword_meta'=>$keywordMeta]);
        }else{
            $update1=DB::table('meta_tags')
                ->where('page','default')
                ->insert(['title_meta'=>$titleMeta, 'description_meta'=>$descriptionMeta, 'keyword_meta'=>$keywordMeta]);
        }
       
        Session::flash('success','Default Meta Updated Successfully');
        return redirect()->back();
    }
