...
class mainController extends Controller
{
    ...
    public function generateDynamicXml(){
        ...
        // $i=1;
        // echo '<br>fuer<br>';
        // $fauerA=DB::select(DB::raw(
        //     'select * from fauer'
        // ));
        // foreach ($fauerA as $fas) {
        //     //echo "fauer".$fas->fauerAvailable;
        //     $link=$base."fuer/".$fas->fauerAvailable;
        //     //echo $masterI.'  '.$i." ".$link.'<br>';
        //     $finalXmlArr[$masterI]["link"]=$link;
        //     $finalXmlArr[$masterI]["priority"]='0.5';
        //     $finalXmlArr[$masterI]["time"]=date('c', time());
        //     $finalXmlArr[$masterI]["frequency"]="monthly";
        //     $masterI++;
        //     $i++;
        ...
        //         foreach ($fauerA as $fas) {
        //             //echo "fauer".$fas->fauerAvailable;
        //             $linkF=$link.'/'."fuer/".$fas->fauerAvailable;
        //             //echo $masterI.'  '.$i." ".$linkF.'<br>';
        //             $finalXmlArr[$masterI]["link"]=$linkF;
        //             $finalXmlArr[$masterI]["priority"]='0.6';
        //             $finalXmlArr[$masterI]["time"]=date('c', time());
        //             $finalXmlArr[$masterI]["frequency"]="monthly";
        //             $masterI++;
        //             $i++;
        ...
        // }
        // $i=1;
        // echo '<br>Combined category region fauer<br>';
        // $sitemapContent=DB::select(DB::raw(
        //     'select * from tx_realurl_uniqalias where (tablename="tx_travel_domain_model_category") AND field_alias="title_url"'
        // ));
        // foreach ($sitemapContent as $allLinks) {
        ...
        //                         foreach ($fauerA as $fas) {
        //                             //echo "fauer".$fas->fauerAvailable;
        //                             $linkF=$linkB.'/'."fuer/".$fas->fauerAvailable;
        //                             //echo $masterI.'  '.$i." ".$linkF.'<br>';
        //                             $finalXmlArr[$masterI]["link"]=$linkF;
        //                             $finalXmlArr[$masterI]["priority"]='0.5';
        //                             $finalXmlArr[$masterI]["time"]=date('c', time());
        ...
        $masterI++;
        $i++;
        // $finalXmlArr[$masterI]["link"]='https://www.meinweekend.ch/newsletter';
        // $finalXmlArr[$masterI]["priority"]='1.0';
        // $finalXmlArr[$masterI]["time"]=date('c', time());
        // $finalXmlArr[$masterI]["frequency"]="monthly";
        // $masterI++;
        ...
        $masterI++;
        $i++;
        // $finalXmlArr[$masterI]["link"]='https://www.meinweekend.ch/kontakt';
        // $finalXmlArr[$masterI]["priority"]='1.0';
        // $finalXmlArr[$masterI]["time"]=date('c', time());
        // $finalXmlArr[$masterI]["frequency"]="monthly";
        // $masterI++;
        ...
        $content = View::make('xml', array('finalXmlArr'=>$finalXmlArr))->render();
        $path = app_path().'/'.'../../public_html/sitemap.xml';  
    ...
    public function getHome(Request $request,$parent=1){
        ...
        $defaultMetaData = DB::table('meta_tags')->where('page','default')->first();
        $defaultMetaDataTitle       = isset($defaultMetaData->title_meta) ? $defaultMetaData->title_meta : '';
        $defaultMetaDataKeyword     = isset($defaultMetaData->keyword_meta) ? $defaultMetaData->keyword_meta : '';
        $defaultMetaDataDescription = isset($defaultMetaData->description_meta) ? $defaultMetaData->description_meta : '';

        $title                      =      $defaultMetaDataTitle;
        //$meta['keywords']           =      $defaultMetaDataKeyword;
        $meta['description']        =      $defaultMetaDataDescription;

        $seo=DB::table('category')->where('uid',$pid)->value('seo');
        $seo = str_replace("<br>",'<br/><br/>',$seo);
        $seo=explode('###COLUMNS###',$seo);

        $title                      =     DB::table('category')->where('uid',$pid)->value('title');
        $meta['description']        =     DB::table('category')->where('uid',$pid)->value('description_meta');
        $data['title_tag']          =     DB::table('category')->where('uid',$pid)->value('title_tag');
        ...
        // return $data['categoryData'];
            ...
            if(\Request::segment(1)==''){
                $homeMetaData            = DB::table('meta_tags')->where('page','')->first();
                $homeMetaDataTitle       = isset($homeMetaData->title_meta) ? $homeMetaData->title_meta : '';
                $homeMetaDataKeyword     = isset($homeMetaData->keyword_meta) ? $homeMetaData->keyword_meta : '';
                $homeMetaDataDescription = isset($homeMetaData->description_meta) ? $homeMetaData->description_meta : '';
                $seo[0]                     = isset($homeMetaData->seo) ? $homeMetaData->seo : '';
                $title               =  ($homeMetaDataTitle!='') ? $homeMetaDataTitle : $defaultMetaDataTitle;
                $meta['keywords']    =  ($homeMetaDataKeyword!='') ? $homeMetaDataKeyword : $defaultMetaDataKeyword;
                $meta['description'] =  ($homeMetaDataDescription!='') ? $homeMetaDataDescription : $defaultMetaDataDescription;
            }
    ...
    public function booking02(Request $request){
     ...
     public function redirect4(Request $request, $parent,$category,$region,$day,$search){
        ...
        return $this->listOffers($request, $region,$category,$day,$search,$v);
     }
     ...
     public function redirect33(Request $request,$parent,$category,$region,$fuer){
         $v=$this->getParent($parent);
         return $this->listOffers($request,$region,$category,$fuer,'alle',$v);
     }

    public function redirect40(Request $request,$parent,$region){
        $v=$this->getParent($parent);
        return $this->listOffers($request,$region,$parent,'alle','alle',$v);
        
    }
    public function redirect42(Request $request,$parent,$region,$day,$search){
        $v=$this->getParent($parent);
        return $this->listOffers($request,$region,$parent,$day,$search,$v);
        
    }
    public function redirect43(Request $request,$parent,$region,$search){
        $v=$this->getParent($parent);
        return $this->listOffers($request,$region,$parent,'alle',$search,$v);
        
    }

    public function redirect44(Request $request,$parent,$search){
        $v=$this->getParent($parent);
        return $this->listOffers($request,'alle',$parent,'alle',$search,$v);
    }
    ...
    public function redirect48(Request $request,$parent,$region,$day){
        $v=$this->getParent($parent);
        return $this->listOffers($request,$region,$parent,$day,'alle',$v);
    }
    ...
    public function redirect56(Request $request,$region){
        $parent=str_replace('.html','',$region);
        return $this->listOffers($request,$region,'alle','alle','alle');
    ...
    public function getOfferPageSEO(Request $request,$offerUrl,$callBack=0){
        ...
        $data['singleOfferData']=DB::table('offer')
            ...
            ->get();
        $title=$data['singleOfferData'][0]->title.'-meinweekend.ch';    
        $meta['description']=$data['singleOfferData'][0]->meta_description;
        // $meta['title']=$data['singleOfferData'][0]->title;  
        // $meta['image']='https://www.meinweekend.ch/fileadmin/img/logo.png'; 
        // $meta['url']="https://www.meinweekend.ch/".$offer_id."/".str_replace(" ","-",$data['singleOfferData'][0]->title);   
    ...
    public function checkDates($offer){
       ...
       //get list of closed and open days
        if(array_sum($available)>0 || strlen($open_dates)>0){
    ...
    public function listOffers(Request $request,$region_link='alle',$offer_type_link='alle',$duration_link='alle',$keyword_link='alle',$mainCat=0){
    ...
    public function editOfferForm($offer_id){
        ...
        $prices=DB::table('price')
            ...
            ->select('price.*','options.title as otitle')
            ->orderBy('sorting')
            ->get();
        // if(count($prices)==0){
        //     return $this->editOfferForm($offer_id+1);
    ...
    public function offerVaucherPage(Request $request,$offerId,$callBack=0){
    ...
    public function offerVaucherPage2(Request $request,$vid){
        return redirect('/angebote/geschenkgutschein-geschenkidee/ideen/'.$vid);
    }
    ...
    public function submitVaucherStepOne(Request $request){
        ...
        //var_dump($request->imgForm, $request->st2Price);
    ...
    //     $titles=DB::Select(DB::raw('UPDATE images2 set title=(select sys_file_reference.title from sys_file_reference left join sys_file_reference on sys_file_reference.uid_local=sys_file.uid where identifier=images2.identifier)'));
    //     return 'Nice';
    // }
    public function redirectOfferVaucher(Request $request,$vid){
        return redirect('/angebote/geschenkgutschein-geschenkidee/ideen/'.$vid);
    }
    ...
    public function homeMeta(Request $request){
        $data=DB::table('meta_tags')->where('page','')->first();
        return view('homeMeta')->with('data',$data);
    }

    public function defaultMeta(Request $request){
        $data=DB::table('meta_tags')->where('page','default')->first();
        return view('defaultMeta')->with('data',$data);
    }
