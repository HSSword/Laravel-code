...
class Controller extends BaseController
{
    ...
    // public function offerImagesTransfer(){
    //     $data=DB::table('sys_file_reference')
    //         ->join('sys_file','sys_file.uid','sys_file_reference.uid_local')
    //         ->where([['identifier','like','%tx_travel/%'],['sys_file.missing',0],['sys_file_reference.deleted',0]])
    //         ->select('sys_file_reference.sorting_foreign','identifier','sys_file_reference.uid_foreign')
    //         ->get();
    //     $dataToInsert=[];
    //     foreach($data as $d){
    //         $dataToInsert[]=['offer'=>$d->uid_foreign,'image'=>$d->identifier,'deleted'=>0,'hidden'=>0,'sorting'=>$d->sorting_foreign];
    //     }
    //     return DB::table('images2')->insert($dataToInsert);
   ...
   public function fixBookings(){
    ...
    public function stats(){
        // $minAndMax=DB::Select(DB::raw('SELECT min(crdate) as min, max(crdate) as max from purchases'));
        // for($i=$minAndMax[0]->min;$i<$minAndMax[0]->max;$i+=86400){
        //     $dates[Carbon::createFromTimeStamp($i)->format('d.m.Y')]=DB::table('purchases')->whereBetween('crdate',[$i,$i+86400])->count();
        // }
